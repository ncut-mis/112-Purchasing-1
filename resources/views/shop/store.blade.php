<x-app-layout>
    ...
    <div x-data="{ 
        ...
        getRatingLabel(rating, count) {
            ...
            return '平均評價 ' + this.getRatingScore(rating, count);
        },
                ...
                @forelse($visibleAgents->values() as $index => $agent)
                    <div x-show="{{ $index }} < limit"
                        ...
                        <div class="flex flex-col items-center text-center flex-1">
                            ...
                            @endphp
                            <div class="mt-3 flex items-center justify-center gap-2" aria-label="{{ $agent->reviews_count ? '平均評價 '.$ratingText : '尚無評價' }}">
                                ...
                                <span class="inline-flex items-center rounded-full bg-amber-50 px-2.5 py-1 text-xs font-black text-amber-700 ring-1 ring-amber-100">評價 {{ $ratingText }}</span>
                            </div>
                ...
                <div class="overflow-y-auto flex-1 custom-scrollbar">
                    <div class="bg-emerald-800 p-8 pt-12 pb-16 relative text-white flex flex-col md:flex-row items-center md:items-end gap-6">
                        ...
                        <div class="text-center md:text-left flex-1 pb-2">
                            ...
                            <div class="mt-3 flex items-center justify-center md:justify-start gap-2 text-amber-300" :aria-label="getRatingLabel(activeAgent?.average_rating, activeAgent?.reviews_count)">
                                ...
                                <span class="inline-flex items-center rounded-full bg-amber-400/20 px-2.5 py-0.5 text-xs font-black text-amber-200 ring-1 ring-amber-300/30"
                                      x-text="'評價 ' + getRatingScore(activeAgent?.average_rating, activeAgent?.reviews_count)"></span>
                            </div>
                        </div>
