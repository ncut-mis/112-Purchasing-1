<x-app-layout>
    <!-- 引入 Bootstrap 5 CSS 與圖示 (確保樣式正確顯示) -->
                        ...
                        <section class="col-12 col-lg-8 col-xl-9 d-flex flex-column bg-white">
                            ...
                            <form id="chat-form" class="border-top p-3 bg-light-subtle d-flex gap-2 align-items-center">
                                <input
                                    ...
                                    autocomplete="off"
                                >
                                <button type="submit" class="btn btn-success rounded-pill px-4 d-flex align-items-center gap-2 text-nowrap">
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function renderUsers() {
                ...
                visibleUsers.forEach((user) => {
                    ...
                    button.type = 'button';
                    button.className = `btn text-start border rounded-4 p-3 mb-2 w-100 ${isActive ? 'border-success-subtle bg-success-subtle' : 'bg-white hover-shadow-sm'}`;

                    button.innerHTML = `
                        <div class="d-flex justify-content-between align-items-start gap-2">
                            <div class="d-flex gap-2 align-items-center min-w-0">
                                <div class="rounded-circle d-flex align-items-center justify-content-center fw-bold border" style="width:42px;height:42px;flex-shrink:0;${isActive ? 'background:#d7f2ea;color:#3d8e7f;' : 'background:#fff;color:#6c757d;'}">
                                    ${user.name.slice(0, 1)}
                                </div>
                                <div class="min-w-0">
            ...
            function renderHeader() {
                ...
                chatHeader.innerHTML = `
                    <div class="d-flex align-items-center gap-3">
                        <div class="rounded-circle d-flex align-items-center justify-content-center fw-bold border" style="width:46px;height:46px;background:#e8f5f2;color:#3d8e7f;flex-shrink:0;">${user.name.slice(0,1)}</div>
                        <div>
            ...
            function renderMessages() {
                ...
                if (!user) {
                    chatMessages.innerHTML = '<div class="text-muted text-center py-5">目前沒有開啟中的聊天。</div>';
                    return;
                }
            ...
            function renderRestoreList() {
                ...
                hiddenUsers.forEach((user) => {
                    ...
                    btn.type = 'button';
                    btn.className = 'btn btn-outline-success text-start rounded-3 d-flex justify-content-between align-items-center mb-2';
                    btn.innerHTML = `
                        <span>${user.name}</span>
