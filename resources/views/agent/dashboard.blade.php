...
<div x-data="{ 
    ...
    submitQuote() {
    // 1. 取得數值並驗證
    const total = parseFloat(this.agentQuoteTotal);
    
    if(!this.agentQuoteTotal || !this.availableTime) {
        alert('請填寫總報價與可代購時段');
        return;
    }
    
    // 2. 準備要傳送的資料物件 (Payload)
    const payload = {
    ...
    fetch('/request-lists/agent-quote', { 
        ...
        // 修正處：直接傳入 payload 變數即可，不要在後面加括號
    ...
    <div class="flex items-center gap-2">
        ...
        @elseif($requestList->people)
            ...
            @if((int)$requestList->people === (int)auth()->id())
                ...
                <div class="flex flex-col items-end mr-2">
                    <span class="text-amber-600 font-black text-sm">正在進行中</span>
                    <span class="text-[10px] text-gray-400">您已承接</span>
                </div>
                <a href="{{ route('messages.index', ['id' => $requestList->id]) }}" 
                   class="px-5 py-2.5 bg-green-500 text-white rounded-xl font-bold hover:bg-green-600 shadow-sm transition-all flex items-center gap-1 text-sm">
                    <i class="bi bi-chat-dots"></i> 聊一聊
                </a>
            @else
                {{-- 別人接的 --}}
                <div class="px-4 py-2 bg-gray-100 rounded-lg border border-gray-200">
                    <span class="text-gray-400 font-medium text-sm">已被他人承接</span>
                </div>
            @endif
        @else
            {{-- 3. 沒人接，開放接單 --}}
            <button type="button" @click="openDetail(@js($orderData))"
                    class="px-8 py-2.5 bg-indigo-600 text-white rounded-xl font-bold text-sm flex items-center gap-2 hover:bg-indigo-700 transition shadow-md active:scale-95">
        ...
        @empty
            <div class="col-span-full text-center py-12 bg-white rounded-3xl border-2 border-dashed border-gray-200">
                <p class="text-gray-400 font-bold">目前沒有符合條件的請購需求</p>
            </div>
        @endforelse
    </div>
</div>
                        ...
                        <div class="p-8 pt-6 border-t bg-gray-50">
                            <div class="flex gap-4">
                                ...
                                <button @click="submitQuote()"
                                    :disabled="!agentQuoteTotal || !availableTime"
                                    :class="(!agentQuoteTotal || !availableTime) ? 'opacity-50 cursor-not-allowed' : 'hover:bg-indigo-700'"
