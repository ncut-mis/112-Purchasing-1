<div class="bg-white rounded-2xl shadow-sm p-6" x-data="{ followTab: 'unpaid' }">
    ...
    <input type="search" name="follow_search" placeholder="搜尋貼文標題、代購人..."
        ...
        class="w-full pl-10 pr-4 py-2 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-[#bb63f1] focus:border-[#bb63f1] shadow-sm transition outline-none">
                            ...
                            {{-- Tab 切換 --}}
                            @php
                                $unpaidOrders          = $followOrders->filter(fn($o) => in_array($o->status, ['pending_payment'], true));
                                $waitingShipmentOrders = $followOrders->filter(fn($o) => in_array($o->status, ['wait-for-ship'], true));
                                $shippedOrders         = $followOrders->filter(fn($o) => in_array($o->status, ['shipped'], true));
                                $arrivedOrders         = $followOrders->filter(fn($o) => in_array($o->status, ['arrivaled'], true));
                            @endphp
                            <div class="flex flex-wrap gap-2 mb-5 border-b border-gray-100 pb-3">
                                <button type="button"
                                ...
                                <button type="button"
                                    @click="followTab = 'waiting'"
                                    :class="followTab === 'waiting' ? 'bg-amber-50 text-amber-700 border-amber-300 font-bold' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50'"
                                    class="px-4 py-1.5 rounded-full border text-xs transition">
                                    待出貨
                                    @if($waitingShipmentOrders->count() > 0)
                                        <span class="ml-1 inline-flex items-center justify-center w-4 h-4 rounded-full bg-amber-500 text-white text-[10px]">{{ $waitingShipmentOrders->count() }}</span>
                                    @endif
                                </button>
                                <button type="button"
                                    @click="followTab = 'shipped'"
                                    :class="followTab === 'shipped' ? 'bg-blue-50 text-blue-700 border-blue-300 font-bold' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50'"
                                    class="px-4 py-1.5 rounded-full border text-xs transition">
                                    已出貨
                                    @if($shippedOrders->count() > 0)
                                        <span class="ml-1 inline-flex items-center justify-center w-4 h-4 rounded-full bg-blue-500 text-white text-[10px]">{{ $shippedOrders->count() }}</span>
                                    @endif
                                </button>
                                <button type="button"
                            ...
                            @if(request('follow_search'))
                                <div class="mb-4 rounded-lg border border-purple-200 bg-purple-50 px-4 py-3 text-sm text-purple-700">
                                    搜尋「{{ request('follow_search') }}」找到 {{ $followOrders->count() }} 筆跟團。
                            ...
                            {{-- 待出貨 --}}
                            <div x-show="followTab === 'waiting'" class="space-y-2.5">
                                @if($waitingShipmentOrders->isEmpty())
                                    <div class="text-gray-400 text-sm text-center py-10 border border-dashed border-gray-200 rounded-xl">目前沒有待出貨的跟團。</div>
                                @else
                                @foreach($waitingShipmentOrders as $followOrder)
                                    @php
                                        $followOrderTitle = $followOrder->source?->title
                                            ?? data_get($followOrder->recipient_data, 'post_title')
                                            ?? '未命名貼文';
                                        $followOrderStatusText = '待出貨';
                                    @endphp
                                    <article class="flex flex-col gap-2.5 rounded-[20px] border border-amber-100 bg-amber-50/40 px-3.5 py-3 shadow-sm lg:flex-row lg:items-center">
                                        <div class="flex h-16 w-16 shrink-0 items-center justify-center rounded-[18px] bg-white text-amber-400 shadow-sm">
                                            <svg class="h-7 w-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path>
                                            </svg>
                                        </div>
                                        <div class="min-w-0 flex-1">
                                            <div class="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
                                                ...
                                                <div class="flex items-center justify-between gap-2 lg:justify-end">
                                                    <span class="inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold bg-amber-50 text-amber-700 border-amber-200">{{ $followOrderStatusText }}</span>
                                                    <button type="button"
                                                        onclick="openFollowOrderModal({{ $followOrder->id }})"
                                    ...
                                    {{-- 檢視 Modal（待出貨） --}}
                                     <div id="follow-order-modal-{{ $followOrder->id }}" class="follow-order-modal hidden fixed inset-0 z-[72] flex items-center justify-center bg-slate-900/55 px-4 py-6" onclick="handleFollowOrderBackdrop(event, {{ $followOrder->id }})">
                                        <div class="w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl">
                                            <div class="flex items-start justify-between gap-3 border-b border-amber-100 bg-amber-50 px-5 py-4">
                                                <div>
                                                    <p class="text-xs font-semibold uppercase tracking-wider text-amber-500">跟團完整資料</p>
                                                    <h4 class="mt-1 text-lg font-bold text-slate-800">{{ $followOrderTitle }}</h4>
                                                </div>
                                                <button type="button" class="rounded-full bg-white px-2.5 py-1 text-slate-500 ring-1 ring-slate-200 transition hover:bg-slate-50" onclick="closeFollowOrderModal({{ $followOrder->id }})" aria-label="關閉">✕</button>
                                            </div>
                                            <div class="max-h-[72vh] overflow-y-auto px-5 py-5">
                                                ...
                                                <div class="grid gap-3 sm:grid-cols-2">
                                                    <div class="rounded-xl bg-slate-50 px-4 py-3"><p class="text-xs text-slate-400">代購人</p><p class="mt-1 text-sm font-semibold text-slate-700">{{ optional($followOrder->seller)->name ?? '-' }}</p></div>
                                                    <div class="rounded-xl bg-slate-50 px-4 py-3"><p class="text-xs text-slate-400">訂單狀態</p><p class="mt-1 text-sm font-semibold text-amber-700">{{ $followOrderStatusText }}</p></div>
                                                    <div class="rounded-xl bg-slate-50 px-4 py-3"><p class="text-xs text-slate-400">下單日期</p><p class="mt-1 text-sm font-semibold text-slate-700">{{ optional($followOrder->created_at)->format('Y-m-d H:i') }}</p></div>
                                                    <div class="rounded-xl bg-slate-50 px-4 py-3"><p class="text-xs text-slate-400">付款方式</p><p class="mt-1 text-sm font-semibold text-slate-700">{{ ['linepay'=>'LINE Pay','bank'=>'超商付款','jkopay'=>'街口支付'][$followOrder->payment_method] ?? ($followOrder->payment_method ?: '未設定') }}</p></div>
                                                    @if($sourcePost && ($sourcePost->start_date || $sourcePost->end_date))
                                                    <div class="rounded-xl bg-slate-50 px-4 py-3"><p class="text-xs text-slate-400">銷售期間</p><p class="mt-1 text-sm font-semibold text-slate-700">{{ optional($sourcePost->start_date)->format('Y-m-d') }} ～ {{ optional($sourcePost->end_date)->format('Y-m-d') }}</p></div>
                            ...
                            {{-- 已出貨 --}}
                            <div x-show="followTab === 'shipped'" class="space-y-2.5">
                                @if($shippedOrders->isEmpty())
                                    <div class="text-gray-400 text-sm text-center py-10 border border-dashed border-gray-200 rounded-xl">目前沒有已出貨的跟團。</div>
                                @else
                                @foreach($shippedOrders as $followOrder)
                                    @php
                                        $followOrderTitle = $followOrder->source?->title
                                            ?? data_get($followOrder->recipient_data, 'post_title')
                                            ?? '未命名貼文';
                                        $followOrderStatusText = '已出貨';
                                    @endphp
